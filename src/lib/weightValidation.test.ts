...
describe('weightValidation - límite del peso total según la fórmula', () => {
  ...
  it('redondea la suma a 2 decimales (sin ruido de punto flotante)', () => {
    const result = validateWeights([0.1, 0.2], '0-to-max', 5);
    expect(result.totalWeight).toBe(0.3);
    expect(result.exceeds).toBe(false);
  });

  it('no marca como excede por ruido de punto flotante cerca del límite', () => {
    const result = validateWeights([0.7, 0.7, 0.7, 0.7, 0.7], '0-to-max', 5);
